...
def test_clean_inplace_inflect_graph_compilation_932_states():
    """Compiles open inflect graph with parC and verifies exact 1753 states."""
    ...
    assert inflect_no_infer.num_states() == 1753
    ...
    assert inflect_infer.num_states() == 1753


def test_clean_inplace_parse_graph_compilation_932_states():
    """Compiles open parse graph with parC and verifies exact 1753 states."""
    # Compile with infer_lexical_features=False
    ...
    assert parse_no_infer.num_states() == 1753
    ...
    assert parse_infer.num_states() == 1753
...
def test_clean_inplace_roundtrip_inflection_and_parse():
    ...
    tag_str = "[PrefixClass=a_stem][Pro=1sg.A][H_metathesis=none][H_alt=none][AspectClass=a][Aspect=present][Tense=present_a]"
