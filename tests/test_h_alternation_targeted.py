...
def test_h_drop_entry_22():
    ...
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg.A][H_DROP]atawhahthvhit[AspectClass=be-at][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "katawahthvhitoha" in forms


def test_h_drop_entry_214():
    """Entry 214: [Pro][H_DROP]whahthvhit[Aspect][Tense] -> 1sg tsiwahthvhitoha
    1st h in wh drops, subsequent ah, th, vh remain.
    """
    tag_str = "[PrefixClass=cons_stem][Pro=1sg>3sg][H_DROP]whahthvhit[AspectClass=be-at][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiwahthvhitoha" in forms


def test_h_drop_entry_218():
    """Entry 218: [Pro][H_DROP]yhahkhets[Aspect][Tense] -> 1sg tsiyahkhetska
    1st h in yh drops, subsequent hk remains.
    """
    tag_str = "[PrefixClass=cons_stem][Pro=1sg>3sg][H_DROP]yhahkhets[AspectClass=stative-k][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyahkhetska" in forms


def test_h_drop_entry_537():
    """Entry 537: [Pro][H_DROP]yvwhahth[Aspect][Tense] -> 1sg tsiyvwahthiha
    1st h in wh drops, subsequent th remains.
    """
    tag_str = "[PrefixClass=cons_stem][Pro=1sg>3sg][H_DROP]yvwhahth[AspectClass=ih-vh[inf2]][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyvwahthiha" in forms


def test_h_glot_entry_65():
    """Entry 65: [Pro][H_GLOT]atehohist[Aspect][Tense] -> 1sg tsiyate'ohistiha
    1st h becomes ', 2nd h in histiha remains.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg>3sg][H_GLOT]atehohist[AspectClass=cause][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyate'ohistiha" in forms


def test_h_glot_entry_321():
    """Entry 321: [Pro][H_GLOT]alihelitsh[Aspect][Tense] -> 1sg tsiyali'elitsheha
    1st h becomes ', subsequent tsh remains.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg>3sg][H_GLOT]alihelitsh[AspectClass=apl[imp3]][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyali'elitsheha" in forms


def test_h_glot_entry_1045():
    """Entry 1045: [Pro][H_GLOT]ohiyhtan[Aspect][Tense] -> 1sg tsiyo'iyhtaneha
    1st h becomes ', subsequent yht remains.
    """
    tag_str = "[PrefixClass=vowel_stem][Pro=1sg>3sg][H_GLOT]ohiyhtan[AspectClass=apl][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyo'iyhtaneha" in forms


def test_h_lat_entry_175():
    """Entry 175: [Pro][H_LAT]alhawitht[Aspect][Tense] -> 1sg tsiyatlawithtiha
    1st lh becomes tl, subsequent th and ht remain.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg>3sg][H_LAT]alhawitht[AspectClass=cause[perf3]][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tsiyatlawithtiha" in forms


def test_h_lat_entry_173():
    """Entry 173: [Pro][H_LAT]alhawit[Aspect][Tense] -> 1sg katlawitiha
    1st lh becomes tl.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg.A][H_LAT]alhawit[AspectClass=ih-vh][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "katlawitiha" in forms


def test_h_lat_entry_563():
    """Entry 563: [DIST=de][Pro][H_LAT]alhilost[Aspect][Tense] -> 1sg tetsiyatlilostiha
    1st lh becomes tl.
    """
    tag_str = "[DIST=de][PrefixClass=a_stem][Pro=1sg>3sg][H_LAT]alhilost[AspectClass=cause][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "tetsiyatlilostiha" in forms


def test_h_vowel_entry_45():
    """Entry 45: [Pro][H_VOWEL]atanhthehil[Aspect][Tense] -> 1sg katanvthehilo'a
    Vowel v restored at 1st h, subsequent th and hil remain.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg.A][H_VOWEL]atanhthehil[AspectClass=o][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "katanvthehilo'a" in forms


def test_h_vowel_entry_186():
    """Entry 186: [Pro][H_VOWEL]alhkhotht[Aspect][Tense] -> 1sg kalikhothtiha
    Vowel i restored in lh, subsequent kh and th remain.
    """
    tag_str = "[PrefixClass=a_stem][Pro=1sg.A][H_VOWEL]alhkhotht[AspectClass=cause][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
    assert "kalikhothtiha" in forms


def test_h_vowel_entry_280():
    """Entry 280: [Pro][H_VOWEL]khtha[Aspect][Tense] -> 1sg tsikathaha
    Vowel a restored in kh, subsequent th remains.
    """
    tag_str = "[PrefixClass=cons_stem][Pro=1sg>3sg][H_VOWEL]khtha[AspectClass=stative-h][Aspect=present][TenseClass=a_present][Tense=present]"
    forms = _inflect(tag_str)
