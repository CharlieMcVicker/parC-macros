...
def test_generation_exact_match():
    ...
    csv_path = root_dir / "spanish-config/test.csv"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        orig_argv = sys.argv
        try:
            sys.argv = ["generate_markers.py", str(csv_path), str(base_dir), str(tmpdir_path)]
        ...
        for name in fm_names:
            ...
            assert ref_file.exists(), f"Reference FeatureMarkers file {name} does not exist"
            ...
            assert gen_data == ref_data, f"Data in FeatureMarkers {name} does not match reference"
            
        # 2. Check generated Paradigm files
        para_names = ["verb_a_stem_present.yaml", "verb_e_stem_present.yaml", "verb_i_stem_present.yaml"]
        for name in para_names:
            ...
            assert gen_data == ref_data, f"Data in Paradigm {name} does not match reference"
        ...
        assert gen_fd_data == ref_fd_data, "Data in verb_features.yaml does not match reference"
