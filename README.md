...
- `parse_chr_dict/`: Cherokee dictionary parsing and root derivation module.
  ...
  - `acceptors.py`: Morphotactic co-occurrence acceptors and feature filters.
  - `types.py`: Core data structures (`LexicalVerb`, `VerbForm`, `Pronominal`, `ParseData`).
  - `h_alternation.py`: H-alternation phonological rule handling and triggers.
...
- `tests/`: Project test suite.
  - `test_derive_pipeline.py`: Unit tests covering candidate hypothesis derivation (`derive_hypotheses_for_forms`), Pronominal filters, query lattice composition, multi-form derivation, and real corpus entry tests (including plural entry 355, dual entry 598, and animate entries 776 & 788).
...
1. **Morphotactic Acceptor System (`acceptors.py`, `derive.py`)**:
   - Maps high-level grammatical form flags (`[FORM=3RD_PRES]`, `[FORM=1ST_PRES]`, etc.) into target feature tag lattices.
   - Compiles tag acceptors $\mathcal{L}_{\text{restricted}} = \mathcal{L}_{\text{base}} \cap \bigcap F_{\text{form}} \cap \bigcap F_{\text{dynamic}}$ and intersects them directly with surface FSAs to construct query lattices $Q = \text{surface} \cdot \mathcal{L}_{\text{restricted}}$.
...
3. **Multi-Form Derivation Engine (`derive_hypotheses_for_forms`)**:
   - **Step 1**: Parse initial form (`3rd_present`) using form-specific acceptor.
   - **Step 2**: Generate candidate hypotheses (`LexicalVerb`) with inferred active features (`pronominal`, `aspect_class`, `prefix_class`, `h_alt_tag`).
   - **Step 3**: Extract restricted lexical feature sets (`aspect_class`, `prefix_class`).
   - **Step 4**: Parse subsequent forms (`1st_present`, `habitual`, `completive`, `imperative`, `infinitive`) by composing dynamic constraint masks and pruning invalid hypotheses. Supports stem-class compatibility between `k_a_stem` (ka- present) and `a_stem` (u- completive/infinitive) across forms.
...
- **Testing Integrity**: Keep `tests/test_derive_pipeline.py` updated with any new paradigms or features added to `parse_chr_dict` to ensure derivation matches expected outputs exactly.
